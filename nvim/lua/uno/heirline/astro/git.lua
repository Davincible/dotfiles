...
--- Run a git command from the unoline installation directory
-- @param args the git arguments
-- @return the result of the command or nil if unsuccessful
function git.cmd(args, ...)
	return unoline.cmd("git -C " .. unoline.install.home .. " " .. args, ...)
end

--- Check if the unoline is able to reach the `git` command
-- @return the result of running `git --help`
function git.available()
	return git.cmd("--help", false)
end

--- Check if the unoline home is a git repo
...
function git.remote_url(remote, ...)
	return unoline.trim_or_nil(git.cmd("remote get-url " .. remote, ...))
end
...
function git.current_version(...)
	return unoline.trim_or_nil(git.cmd("describe --tags", ...))
end

--- Get the current branch
-- @return the branch of the unoline installation
function git.current_branch(...)
	return unoline.trim_or_nil(git.cmd("rev-parse --abbrev-ref HEAD", ...))
end

--- Get the current head of the git repo
-- @return the head string
function git.local_head(...)
	return unoline.trim_or_nil(git.cmd("rev-parse HEAD", ...))
end
...
function git.remote_head(remote, branch, ...)
	return unoline.trim_or_nil(git.cmd("rev-list -n 1 " .. remote .. "/" .. branch, ...))
end
...
function git.tag_commit(tag, ...)
	return unoline.trim_or_nil(git.cmd("rev-list -n 1 " .. tag, ...))
end
...
function git.parse_remote_url(str)
	return vim.fn.match(str, unoline.url_matcher) == -1
			and git.url .. str .. (vim.fn.match(str, "/") == -1 and "/unoline.git" or ".git")
			or str
end
...
-- @param commits an array like table of commit messages
-- @return an array like table of echo messages to provide to nvim_echo or unoline.echo
