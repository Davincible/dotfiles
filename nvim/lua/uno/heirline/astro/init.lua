...
unoline.install = unoline_installation or { home = stdpath("config") }
--- external unoline configuration folder
unoline.install.config = stdpath("config"):gsub("nvim$", "unoline")
vim.opt.rtp:append(unoline.install.config)
local supported_configs = { unoline.install.home, unoline.install.config }
...
unoline.user_settings = load_module_file("user.init")
--- default packer compilation location to be used in bootstrapping and packer setup call
unoline.default_compile_path = stdpath("data") .. "/packer_compiled.lua"
--- table of user created terminals
unoline.user_terminals = {}
...
-- @function unoline.func_or_extend
...
local function func_or_extend(overrides, default, extend)
	...
	if extend then
		...
		if type(overrides) == "table" then
			default = unoline.default_tbl(overrides, default)
...
function unoline.default_tbl(opts, default)
	opts = opts or {}
...
function unoline.conditional_func(func, condition, ...)
...
function unoline.trim_or_nil(str)
...
function unoline.pad_string(str, padding)
	padding = padding or {}
...
function unoline.initialize_icons()
	unoline.icons = unoline.user_plugin_opts("icons", require("uno.heirline.icons.nerd_font"))
	unoline.text_icons = unoline.user_plugin_opts("text_icons", require("uno.heirline.icons.text"))
end

--- Get an icon from `lspkind` if it is available and return it
-- @param kind the kind of icon in `lspkind` to retrieve
-- @return the icon
function unoline.get_icon(kind)
	local icon_pack = vim.g.icons_enabled and "icons" or "text_icons"
	if not unoline[icon_pack] then
		unoline.initialize_icons()
	end
	return unoline[icon_pack] and unoline[icon_pack][kind] or ""
end

--- Serve a notification with a title of unoline
...
function unoline.notify(msg, type, opts)
	vim.notify(msg, type, unoline.default_tbl(opts, { title = "unoline" }))
end
...
function unoline.echo(messages)
...
function unoline.confirm_prompt(messages)
	if messages then
		unoline.echo(messages)
	end
...
local function user_setting_table(module)
	...
	local settings = unoline.user_settings or {}
...
function unoline.initialize_packer()
	-- try loading packer
	...
	if not packer_avail then
		...
		unoline.echo({ { "Initializing Packer...\n\n" } })
	...
	if packer_avail then
		...
		local run_me, _ = loadfile(
			unoline.user_plugin_opts("plugins.packer", { compile_path = unoline.default_compile_path }).compile_path
		)
		...
		if run_me then
			...
			unoline.echo({ { "Please run " }, { ":PackerSync", "Title" } })
		end
	end
end
...
function unoline.vim_opts(options)
...
function unoline.user_plugin_opts(module, default, extend, prefix)
...
function unoline.url_opener()
	...
	elseif vim.fn.has("unix") == 1 then
		...
		unoline.notify("gx is not supported on this OS!", "error")
	end
end
...
function unoline.toggle_term_cmd(term_details)
	...
	if unoline.user_terminals[term_key] == nil then
		unoline.user_terminals[term_key] = require("toggleterm.terminal").Terminal:new(term_details)
	end
	-- toggle the terminal
	unoline.user_terminals[term_key]:toggle()
end

--- Add a source to cmp
-- @param source the cmp source string or table to add (see cmp documentation for source table format)
function unoline.add_cmp_source(source)
...
function unoline.get_user_cmp_source(source)
	...
	local priority = unoline.user_plugin_opts("cmp.source_priority", {
		nvim_lsp = 1000,
		luasnip = 750,
		buffer = 500,
		path = 250,
...
function unoline.add_user_cmp_source(source)
	unoline.add_cmp_source(unoline.get_user_cmp_source(source))
end
...
function unoline.which_key_register(mappings, opts)
...
function unoline.null_ls_providers(filetype)
	local registered = {}
	-- try to load null-ls
...
function unoline.null_ls_register(source)
	-- try to load null-ls
...
function unoline.null_ls_sources(filetype, method)
	local methods_avail, methods = pcall(require, "null-ls.methods")
	return methods_avail and unoline.null_ls_providers(filetype)[methods.internal[method]] or {}
end
...
function unoline.alpha_button(sc, txt)
...
function unoline.is_available(plugin)
...
function unoline.set_mappings(map_table, base)
...
function unoline.delete_url_match()
...
function unoline.set_url_match()
	unoline.delete_url_match()
...
function unoline.cmd(cmd, show_error)
...
require("uno.heirline.astro.ui")
require("uno.heirline.astro.status")
-- require("uno.heirline.astro.updater")
require("uno.heirline.astro.lsp")
require("uno.heirline.astro.diagnostics")
